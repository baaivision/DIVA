...
class VisionTransformer(nn.Module):
    ...
    # DIVA's condition for SigLIP ViT-SO-14/224
    def forward_head(self, x, pre_logits: bool = False):
        tokens = x
        if self.attn_pool is not None:
            ...
            average_pool_window_size = 6
            average_pooled_tokens = F.avg_pool1d(tokens, kernel_size = average_pool_window_size, stride = average_pool_window_size)
            average_pooled_tokens = average_pooled_tokens.permute(0,2,1)
            x = torch.cat((pooled, average_pooled_tokens), dim=1)
        elif self.global_pool == 'avg':
            x = x[:, self.num_prefix_tokens:].mean(dim=1)
        elif self.global_pool:
            x = x[:, 0]  # class token
        ...
        x = self.fc_norm(x)
        x = self.head_drop(x)
        return x.reshape(B,N,C) if pre_logits else self.head(x).reshape(B,N,C)

    # DIVA's condition for SigLIP ViT-SO-14/384
    # def forward_head(self, x, pre_logits: bool = False):
    #     tokens = x
    #     if self.attn_pool is not None:
    #         pooled = self.attn_pool(x)
