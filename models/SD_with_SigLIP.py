...
from .build import load_sd_model, load_clip_model_SigLIP
...
class SDModel(PreTrainedModel):
    def __init__(
        ...
        self.class_model = load_clip_model_SigLIP(config)
        ...
        self.visual_proj = nn.Linear(1152, 1024)
    ...
    def zeroshot_classifier_SigLIP(self, classnames, templates, model):
        with torch.no_grad():
            zeroshot_weights = []
            if self.config.clip_image_size == 224:
                tokenizer = get_tokenizer('ViT-SO400M-14-SigLIP')
            if self.config.clip_image_size == 384:
