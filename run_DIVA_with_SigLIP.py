...
def main():
    ...
    from models.SD_with_SigLIP import SDModel
...
def benchmark_model(model, benchmark_dir, device = "cpu", config=None):
    if config.clip_image_size == 224:
        _, preprocess = create_model_from_pretrained(model_name='ViT-SO400M-14-SigLIP', pretrained="pretrained_weights/CLIP/ViT-SO400M-14-SigLIP/open_clip_pytorch_model.bin", device=device,
                                                     image_mean=([0.5,0.5,0.5]), image_std=([0.5,0.5,0.5]), image_interpolation="bicubic", image_resize_mode="squash")
        tokenizer = get_tokenizer('ViT-SO400M-14-SigLIP')
    if config.clip_image_size == 384:
        _, preprocess = create_model_from_pretrained(model_name='ViT-SO400M-14-SigLIP-384', pretrained="pretrained_weights/CLIP/ViT-SO400M-14-SigLIP-384/open_clip_pytorch_model.bin", device=device,
                                                     image_mean=([0.5,0.5,0.5]), image_std=([0.5,0.5,0.5]), image_interpolation="bicubic", image_resize_mode="squash")
    ...
    csv_outfile = open('Prediction_Results_SigLIP', 'w', newline='')
    ...
    with open(csv_file, 'r') as f:
        ...
        for i, row in tqdm(enumerate(reader)):
            ...
            text1 = tokenizer(text1, context_length=model.context_length).to(device)
            text2 = tokenizer(text2, context_length=model.context_length).to(device)
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                ...
                logits_per_image1 = image_features @ text1_features.T * model.logit_scale.exp() + model.logit_bias
                logits_per_text1 = logits_per_image1.T
                logits_per_image2 = image_features @ text2_features.T * model.logit_scale.exp() + model.logit_bias
...
def official_evaluation(clip_model, config):
    ...
    with torch.no_grad():
        ...
        if config.clip_image_size == 224:
            results_openai = {f'ViT-SO400M-14-SigLIP': benchmark_model(clip_model, data, clip_model_device, config)}
        if config.clip_image_size == 384:
            results_openai = {f'ViT-SO400M-14-SigLIP-384': benchmark_model(clip_model, data, clip_model_device, config)}
