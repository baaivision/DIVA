...
def main():
    ...
    from models.SD_with_MetaCLIP import SDModel
    ...
    config.metaclip_version = training_args.metaclip_version
...
def benchmark_model(model, benchmark_dir, device = "cpu", config=None):
    ...
    if config.metaclip_version == "large":
        _, _, preprocess = create_model_and_transforms(model_name='ViT-L-14-quickgelu', pretrained="pretrained_weights/CLIP/MetaCLIP/l14_fullcc2.5b.pt")
    if config.metaclip_version == "huge":
        _, _, preprocess = create_model_and_transforms(model_name='ViT-H-14-quickgelu', pretrained="pretrained_weights/CLIP/MetaCLIPz/h14_fullcc2.5b.pt")

    image_dir = os.path.join(benchmark_dir, 'MLLM_VLM_Images')
    csv_file = os.path.join(benchmark_dir, 'Questions.csv')

    csv_outfile = open('Prediction_Results_MetaCLIP', 'w', newline='')
    ...
    with open(csv_file, 'r') as f:
        ...
        for i, row in tqdm(enumerate(reader)):
            ...
            text1 = tokenize(text1).to(device)
            text2 = tokenize(text2).to(device)
...
def official_evaluation(clip_model, config):
    ...
    with torch.no_grad():
        ...
        if config.metaclip_version == "large":
            results_openai = {f'MetaCLIP-ViT-L-14': benchmark_model(clip_model, data, clip_model_device, config)}
        if config.metaclip_version == "huge":
            results_openai = {f'MetaCLIP-ViT-H-14': benchmark_model(clip_model, data, clip_model_device, config)}
